...
import org.swtchartplus.IDisposeListener;
import org.swtchartplus.ILineSeries;
import org.swtchartplus.ISeries;
import org.swtchartplus.LineStyle;
...
public class SeriesPage extends AbstractSelectorPage {
    ...
    /** the states indicating the visibility of series of the label*/
    protected boolean[] visibleStatesLabel;
    ...
    public SeriesPage(InteractiveChart chart, PropertiesResources resources, String title) {
        ...
        visibleStatesLabel = new boolean[series.length];
    ...
    protected void selectInitialValues() {
        for (int i = 0; i < series.length; i++) {
            ...
            visibleStatesLabel[i] = series[i].getLabel().isVisible();
    ...
    protected void updateControlSelections() {
    	showLabelButton.setSelection(visibleStatesLabel[selectedIndex]);
    	setControlsLabelEnable(visibleStatesLabel[selectedIndex]);
    ...
    protected void addRightPanelContents(Composite parent) {
    	...
    	showLabelButton = createCheckBoxControl(parent, "Show data point values");
    	//showLabelButton = new Button(parent, SWT.CHECK);
    	//showLabelButton.setText("Show label");
        showLabelButton.getParent().pack();
        showLabelButton.getParent().setLocation(8,0);
        showLabelButton.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                boolean visible = showLabelButton.getSelection();
                visibleStatesLabel[selectedIndex] = visible;
                setControlsLabelEnable(visible);
                apply();
            }
        });

        visibleButton = createCheckBoxControl(parent, "Show plot");
        visibleButton.getParent().pack();
        visibleButton.getParent().setLocation(8,90);
    ...
    private void addSeriesGroup(Composite parent) {
        ...
        /*showLabelButton = createCheckBoxControl(comp, "Show label");
        
        GridData gridData = new GridData();
        gridData.horizontalSpan = 2;
        showLabelButton.getParent().setLayoutData(gridData);
        showLabelButton.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                boolean visible = showLabelButton.getSelection();
        ...
        colorLabel = createLabelControl(comp, "Color:");
        colorButton = createColorButtonControl(comp);
        ...
        stackedButton = createCheckBoxControl(parent, "Stacked series");
    ...
    private void addLineSeriesGroup(Composite parent) {
        lineSeriesGroup = createGroupControl(parent, "", true);
        //lineSeriesGroup = new Group(parent, SWT.NULL);
        //((Group)lineSeriesGroup).setText("Line series:");
        //lineSeriesGroup.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
        
        //comp.setLayout(new GridLayout(1, true));
        
        stackLayout.topControl = lineSeriesGroup;
        
        Composite comp = new Composite(lineSeriesGroup, SWT.NONE);
        GridLayout gridLayout = new GridLayout();
        gridLayout.numColumns = 2;
        comp.setLayout(gridLayout);
        
        GridData gridDatac = new GridData(GridData.HORIZONTAL_ALIGN_FILL);
        gridDatac.horizontalSpan = 2;
        comp.setLayoutData(gridDatac);
                
        GridData gridData = new GridData(GridData.HORIZONTAL_ALIGN_FILL);
        gridData.horizontalIndent = 0;// 20;
        
        /*visibleButton = createCheckBoxControl(comp, "Show plot");
        visibleButton.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                boolean visible = visibleButton.getSelection();
        ...
        /*
        //stackedButton.setLayoutData(gridData);
        
        stackedButton = new Button(comp, SWT.CHECK);
        //stackedButton.setLayoutData(gridData);
        Label l = new Label(comp, SWT.NONE);
        l.setText("Stacked series");*/
           
        createLabelControl(comp, "Line color:");
        lineColorButton = createColorButtonControl(comp);
        ...
        createLabelControl(comp, "Line style:").setLayoutData(gridData);
        ...
        lineStyleCombo = createComboControl(comp, labels);
        ...
        createLabelControl(comp, "Symbol size:");
        symbolSizeSpinner = createSpinnerControl(comp, 1, 10);
        ...
        createLabelControl(comp, "Symbol color:");
        symbolColorButton = createColorButtonControl(comp);
        ...
        createLabelControl(comp, "Symbol type:").setLayoutData(gridData);
        PlotSymbolType[] types = PlotSymbolType.values();
        labels = new String[types.length];
        for (int i = 0; i < types.length; i++) {
            labels[i] = types[i].label;
        }
        symbolTypeCombo = createComboControl(comp, labels);
    ...
    private void addBarSeriesGroup(Composite parent) {
        ...
        barSeriesGroup.pack();
        barSeriesGroup.setLocation(0, 0);
    }
    
    /**
     * Sets the enable state of controls of the label.
     * 
     * @param enabled
     *            true if controls are enabled
     */
    protected void setControlsLabelEnable(boolean enabled) {
    ...
    protected void setControlsEnable(boolean enabled) {
        ...
        symbolColorButton.setEnabled(enabled);
        symbolTypeCombo.setEnabled(enabled);
        symbolSizeSpinner.setEnabled(enabled);
    }
    ...
    public void apply() {
        for (int i = 0; i < series.length; i++) {
        	series[i].getLabel().setVisible(visibleStatesLabel[i]);
    ...
    protected void performDefaults() {
        visibleStatesLabel[selectedIndex] = false;
